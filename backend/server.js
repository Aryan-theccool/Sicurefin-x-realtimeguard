...
let txCounter = 0;
let nextThreatAt = Math.floor(Math.random() * (5 - 3 + 1) + 3);

const startSimulation = () => {
  // Adjusted: ~3 seconds per transaction for faster observation
  const delay = Math.floor(Math.random() * (4000 - 2000 + 1) + 2000);
  setTimeout(() => {
    txCounter++;
    const shouldForceFraud = txCounter >= nextThreatAt;

    if (shouldForceFraud) {
      console.log(`🚀 [SIMULATION] Periodical Threat Triggered! (Tx #${txCounter})`);
    }

    const tx = generateTransaction(shouldForceFraud);

    if (shouldForceFraud) {
      // Reset counter and set next target
      txCounter = 0;
      nextThreatAt = Math.floor(Math.random() * (5 - 3 + 1) + 3);
      // Ensure it is definitely flagged as fraud for the dashboard
